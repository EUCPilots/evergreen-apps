function Get-ProtonMailBridge {
    ...
    foreach ($Category in ($Releases.CategoryName | Select-Object -Unique)) {
        ...
        # Match the file with the identifier "installer" to get the correct download link
        $File = $LatestVersion.File | Where-Object { $_.Identifier -match "installer" }
